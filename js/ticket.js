...
async function init() {
  ...
  productos = filas.map((r) => {
    ...
    const stock = parseInt(r.stock ?? r.cantidad ?? '0', 10);
    return {
      ...
      id: (r.id ?? r.codigo ?? '').trim(),
      stock // ← Nuevo campo numérico
    };
  });
  // 🔍 Filtrar productos con todos los campos obligatorios completos
  // Aquí exijo que el stock sea >= 0 (puedes cambiarlo a > 0 si quieres solo con stock disponible)
